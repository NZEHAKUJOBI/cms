...
{
    Task<PagedResult<InventoryDto>> GetAllAsync(int page, int pageSize, Guid? facilityId, bool? lowStockOnly, string? stateFilter = null);
    ...
    Task<List<InventoryDto>> GetLowStockAlertsAsync(Guid? facilityId, string? stateFilter = null);
    Task<List<InventoryDto>> GetNearExpiryAlertsAsync(int withinDays = 90, Guid? facilityId = null, string? stateFilter = null);
    ...
    Task<RiskSummaryDto> GetRiskSummaryAsync(Guid? facilityId = null, string? stateFilter = null);
}
